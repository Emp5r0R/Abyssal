...
5. Publish stable product releases only with the known cryptographic limits copied into release notes. Never describe Abyssal as Signal-grade, independently audited, or high-assurance until that work exists. Protocol-v5 releases must call out transactional ratchet rollback on failed payload authentication, queued-lifetime one-time-prekey claims, bounded login and attachment resources, single-session enforcement, and directory-checkpoint limits.
