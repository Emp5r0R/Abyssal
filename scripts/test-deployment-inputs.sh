...
printf 'ABYSSAL_PUBLIC_LOCATORS=["https://alternate.example.com"]\n' >> "$NODE_TEST_ROOT/mirage-server/.env"
expect_failure bash "$NODE_TEST_ROOT/deploy/prepare-runtime-env.sh"
sed -i '/^ABYSSAL_PUBLIC_LOCATORS=/d;s|^ABYSSAL_PUBLIC_URL=.*|ABYSSAL_PUBLIC_URL=|' "$NODE_TEST_ROOT/mirage-server/.env"
printf 'ABYSSAL_PUBLIC_LOCATORS=["https://node.example.com"]\n' >> "$NODE_TEST_ROOT/mirage-server/.env"
bash "$NODE_TEST_ROOT/deploy/prepare-runtime-env.sh"
