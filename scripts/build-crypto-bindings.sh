...
CARGO_NDK_BIN="${CARGO_NDK_BIN:-$(command -v cargo-ndk || true)}"
...
if [[ -z "$CARGO_NDK_BIN" && -x "${HOME:-}/.cargo/bin/cargo-ndk" ]]; then
  CARGO_NDK_BIN="$HOME/.cargo/bin/cargo-ndk"
fi
...
if [[ -z "$CARGO_NDK_BIN" ]]; then
  echo "cargo-ndk is required (set CARGO_NDK_BIN or run cargo install cargo-ndk)." >&2
  exit 1
fi
...
cargo build --manifest-path "$ROOT_DIR/Cargo.toml" --package abyssal-core --release
"$ROOT_DIR/target/release/uniffi-bindgen" generate \
  ...
  --metadata-no-deps \
  --language kotlin \
  --config "$ROOT_DIR/rust-core/uniffi.toml" \
...
ANDROID_NDK_HOME="$ANDROID_NDK_HOME" "$CARGO_NDK_BIN" \
  --manifest-path "$ROOT_DIR/Cargo.toml" \
  -t arm64-v8a \
  -t armeabi-v7a \
  -t x86 \
  ...
  build --package abyssal-core --release
