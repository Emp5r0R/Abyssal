...
- Attachment plaintext limits are 20 MiB for images, 100 MiB for videos, and 200 MiB for other files. Bulk attachment format remains cipher v1: a stateless binary XChaCha20-Poly1305 blob with one version byte, a 24-byte nonce, ciphertext, and a 16-byte tag, so the wire body is the plaintext size plus exactly 41 bytes. The bulk key is delivered inside encrypted, ratcheted message metadata; the relay stores and streams only the opaque blob. A default 320 MiB encrypted-RAM quota applies per account in addition to the global RAM limit. One weighted global RAM pool covers stored blobs and in-flight upload/download references, with bounded upload and download concurrency. Android applies an additional heap-aware admission limit before allocating attachment plaintext or ciphertext buffers.
- Upload admission requires the same generated message ID that is bound into the encrypted metadata. The relay stores the encrypted blob in a staged record that is non-downloadable to every user; staged records count against existing byte and record quotas, expire after a 10-minute deadline clamped to final retention, and are swept every 30 seconds. After replay/state acceptance, the exact authenticated owner/chat/message transaction promotes the matching record server-side before fanout and result emission. Rejected, rolled-back, or never-sent messages do not publish it; an accepted message whose result is lost still publishes it. Duplicate live owner/chat/message bindings are rejected without replacing ciphertext. The upload API now requires `message_id`; attachment ciphertext, keys, the cipher-v1 blob, and the protocol-v9 encrypted-message shape are unchanged.
...
- Encrypted text, read-receipt, and attachment-metadata message frames use the same canonical outer envelope in both directions. Each complete UTF-8 application frame occupies the smallest valid bucket of `4096`, `16384`, `65536`, `262144`, or `1048576` bytes and carries random URL-safe filler in `padding_bucket` and `padding`. Relay, web, and Android reject missing or noncanonical padding and strip these transport fields before domain state; all rate, fanout, pending, and outbound-queue budgets count the padded bytes. This does not pad WebSocket control frames or HTTP attachment bodies, hide timing/count/routing/participants/attachment sizes, or add cover traffic. JSON property order is not a security contract, and clients missing this required envelope are wire-incompatible even though the inner crypto remains protocol v9.
...
- WebSockets use a negotiated `abyssal-v1` plus a random 32-byte `ticket.*` subprotocol. Tickets are issued by authenticated `POST /v1/ws-ticket`, expire after 30 seconds, are stored only as digests, and are single-use; bearer tokens are rejected in WebSocket subprotocols. Protocol-v9 E2EE with per-recipient signatures, signed ratchet state, bounded revisions, the canonical outer message-frame buckets, a 608-byte public bundle (stable 64-byte identity portion, 16 canonical 32-byte one-time keys covered by the signed identity bundle, and a 32-byte fallback key), recipient-bound prekey leases, and ratcheted metadata requires Abyssal `2.1.0` clients plus matching web/Android transport implementations. Protocol-v8 shapes/checkpoints fail closed, as do older pre-v8 clients; all are wire-incompatible with v9.
...
The 2026-08-21 integrated gate ran 258 web tests across 19 files, 25 Rust-core tests, 149 relay tests, and 197 Android JVM tests. Web lint/build, Rust formatting and warning-denied Clippy, Android release lint/debug and release compilation, live disposable-relay integration, generated-artifact and deployment checks, and npm/RustSec audits passed. No Android packaging task was invoked; the applicable tests had zero skips, failures, or errors.
...
Android and web use the same Rust core. Account creation/login uses OPAQUE, so the password is not sent as relay application data. Registration also proves possession of the generated Ed25519 identity key over a fresh server challenge and the exact identity upload before the one-time code is consumed. Protocol v9 encrypts text, read receipts, and control metadata with ChaCha20-Poly1305 and carries each recipient's content key through an authenticated `vodozemac` Olm Double Ratchet session. Every recipient envelope contains exactly one Ed25519 signature over the v9 version, authenticated context, common ciphertext, sender identity key, intended username, prekey metadata, and recipient-specific ratchet envelope. The public identity bundle is exactly 608 bytes: a stable 64-byte identity portion, 16 canonical 32-byte one-time keys covered by the signed identity bundle, and one 32-byte fallback key. Plaintexts are padded into authenticated 256-byte length buckets before encryption; this reduces length leakage only to bucket boundaries and is not full traffic padding. The complete encrypted message application frame is then padded to the smallest canonical UTF-8 bucket of `4096`, `16384`, `65536`, `262144`, or `1048576` bytes with random URL-safe filler in `padding_bucket` and `padding`; missing, malformed, wrong-smallest-bucket, truncated, and oversized frames fail closed in relay/web/Android before routing or domain-state installation. This outer transport padding excludes control frames and HTTP attachment bodies, counts in all relevant relay budgets, and does not hide timing, counts, routing, participants, attachment sizes, relay/Cloudflare visibility, or bucket selection. Each successful encrypt/decrypt result also carries an Ed25519-signed ratchet-state snapshot covering the revision, complete encrypted identity state, public bundle, prekey ID, and bounded monotonic revision. Before first-contact encryption, the relay issues an authenticated lease bound to sender, chat, recipient, message, and prekey. Leases have a 30-second idle TTL, a global limit of 4,096, and a per-recipient limit of 16. Sender state commits only after an exact accepted `message_result` for that message ID; the relay preflights all recipients, atomically admits complete fanout, and rolls back pending mutations if admission fails. After an exact rejected or not-sent result, the client releases only its definitely unused lease reservations. A result-sink failure invalidates the session. A recipient publishes decrypted message, control, or read-receipt state only after an exact accepted `ack_result` for the same message ID. The relay sends `ack_result` only after the signed recipient state update and pending removal; the exact transaction removes the consumed lease, adds its one replacement prekey, and preserves other live leases. Rejected, not-sent, timeout, disconnect, malformed, unknown, or duplicate ACK outcomes fail closed. Pending ACKs are bounded separately at 64 on Android and 256 on web. Clients bind lease operations to the active connection generation and release only leases known to be unused; ambiguous outcomes fail closed rather than guessing lease ownership. These results prove delivery/state durability, not that a human read the message. The relay independently verifies state signatures, permits only bounded revision advances and a 128-revision reorder window, and applies accepted state updates atomically. The acknowledgement has a separate action-only Ed25519 signature over the conversation, message, original sender, and consumed prekey. Attachment bulk data remains cipher v1: a version byte, 24-byte nonce, ciphertext, and 16-byte tag. Its 32-byte key is delivered in encrypted ratcheted message metadata, so the relay never receives the attachment key or plaintext. Accepted pending frames pin their matching prekey leases until acknowledgement, eviction, queue removal, or the bounded pending-message TTL. The relay's global encrypted-attachment pool accounts for stored blobs and in-flight references, and expires or bounds every transfer. Presence also carries a stable long-term identity directory checkpoint, while clients pin the long-term identity portion across prekey rotation. The global room catalog is capped at 1,024 entries and each account has a configurable active-room quota. Live outbound queues cap queued bytes at 4 MiB per client and 64 MiB globally. Wipe control sends priority `GLOBAL_WIPE` text and close code `4001`/`purge`; offline clients and bytes already handed to the kernel cannot be recalled. Protocol-v8 shapes/checkpoints fail closed. This is not Signal or MLS: rooms use pairwise fanout, the tested deployment envelope remains at most 117 accounts and the core peer cap is 256, there is no external key-transparency witness, multi-device protocol, persistent rollback anchor, or independent Abyssal audit. Compare direct-chat safety numbers and directory checkpoints out of band. See [SECURITY.md](SECURITY.md).
