...
- Room labels are local to the creating client's live RAM session, not sent to the relay or other participants. Joining clients use the opaque room ID. Labels disappear when the local room handle or process is discarded; shared encrypted room titles are not yet implemented.
