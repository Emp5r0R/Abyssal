...
- Direct chats show `VERIFICATION RECOMMENDED` until the user explicitly confirms that the displayed safety number exactly matches a value obtained through a separate channel. Encrypted direct text, GIF/media upload, attachment view/export/save, and network read receipts remain available before comparison; the confirmation adds a local trust indicator and helps detect relay key substitution. Incoming unverified direct text remains visibly advisory. Confirmation is a user assertion, not independent authentication or a witness: trust stays in RAM for at most 128 peers and is keyed to the direct context, both stable identity prefixes (the first 64 bytes), and the session and connection generations. Rotating prekeys are not part of that key, so prekey rotation does not clear trust. Reconnect, identity change, logout, wipe, expiry, process/page teardown, or bounded-store eviction clears it. Rooms use protocol-v10 MLS; this direct safety-number ceremony does not provide room participant verification or an external transparency witness.
...
- Direct headers display a symmetric safety number derived from both identity keys. Both participants should compare it through a separate trusted channel. Confirmation is advisory and explicit rather than a prerequisite for encrypted sends, attachment access/export, or network read receipts.
...
- Room names are shared inside the owner's MLS-encrypted text and attachment metadata, never in the relay catalog or room creation request. Members learn the name when they receive an owner message and its exact delivery acknowledgement succeeds; until then they see an ID-derived label. Names stay in RAM and disappear when the room handle or process is discarded. Renaming is not supported. See [Encrypted Room Profiles](docs/ENCRYPTED_ROOM_PROFILES.md).
- Attachment uploads use a fixed `/v2/attachment` URL. Routing and retention metadata are in a bounded request-body envelope, not URL query parameters. This reduces access-log exposure, not relay/TLS-terminator visibility. The legacy upload endpoint fails closed; update both clients with the relay. See [upload framing](docs/ATTACHMENT_UPLOAD_V2.md).
...
ABYSSAL_ALLOW_ANDROID_TO_WEB=true \
...
- `ABYSSAL_ALLOW_ANDROID_TO_WEB`: permits Android-origin direct messages, MLS room applications, pending delivery, and attachments to web recipients. Default: `true` for cross-client interoperability; set `false` to isolate Android senders from web recipients.
