...
- Files, images, and videos may only be written to disk through an explicit user save/export flow. Web saves explicitly decrypt authenticated attachments in memory and download them under a sanitized original filename; one-time attachments remain view-only. Android saves attachments as device-bound `.abyssal` AES-GCM export envelopes using Android Keystore, preferring StrongBox when the device supports it. There is not yet an Android in-app import flow, so those exports are archival ciphertext rather than portable files.
