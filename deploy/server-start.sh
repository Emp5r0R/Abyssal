...
if [[ -n "$(docker compose -f deploy/docker-compose.yml ps -q mirage-server)" ]]; then
  echo "Abyssal is already running; startup invites cannot be recovered."
  docker compose -f deploy/docker-compose.yml ps
  curl --fail --silent http://127.0.0.1:4020/health
  echo
  exit 0
fi
ABYSSAL_CONTAINER_UID="$(id -u)" ABYSSAL_CONTAINER_GID="$(id -g)" \
...
echo "Invite capsules print once below. They cannot be retrieved after this attachment closes."
