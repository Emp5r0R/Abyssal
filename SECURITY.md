...
- Android app-owned account, message, endpoint, PIN, password, token, and key state is process memory only. `FLAG_SECURE` blocks normal Android screenshots and recents snapshots. Calculator camouflage secrets are also RAM-only: process death resets the launcher alias and PIN, there is no default PIN, and the user must configure camouflage again after a fresh process.
...
- Login and registration request bodies are capped at 16 KiB, OPAQUE handshakes expire after 60 seconds, and account-code attempts are limited to six per minute. WebSocket frame size/rate, pending queue depth, session inactivity, and per-user room count are bounded.
- Attachment plaintext limits are 20 MiB for images, 100 MiB for videos, and 200 MiB for other files. The relay's serialized upload bound accounts for the 16-byte ChaCha20-Poly1305 tag after unpadded base64 expansion, a 16 KiB fixed envelope budget, and up to 4 MiB of recipient-envelope overhead; it is not a raw-plaintext byte limit. Encrypted attachment RAM is bounded globally and per account (default per-account quota: 320 MiB). Upload request-body reads are capped at two concurrent bodies by default (maximum four), and downloads use a separate bounded concurrency semaphore (default: 2; maximum: 16), so large uploads cannot consume unbounded extraction memory before quota enforcement.
...
Protocol v5 uses pairwise Olm Double Ratchet sessions. Deleted message keys provide the ratchet's forward-secrecy property, and reciprocal DH-ratchet steps provide recovery within an established session after key turnover. Initial asynchronous sessions consume a recipient-specific one-time prekey and rotate it after use. The relay enforces one live WebSocket session per account. A prekey claim expires after 10 minutes only when no matching pending frame remains; a queued frame keeps its claim until acknowledgement, eviction, or queue removal. This prevents a delayed ciphertext from racing a later sender for the same one-time prekey, but it can also keep that prekey unavailable while the frame remains queued. The design is still not MLS: rooms use pairwise fanout, prekey replenishment is one-at-a-time rather than a signed batch service, and multi-device session coordination is not implemented.
